...
export class AuthService {
     ...
     */
    async sign(credentials: { email: string; password: string }): Promise<SignDto> {
        const user = await this.usersService.findOne({email: credentials.email});

        if (!user) {
            throw new HttpException('The specified user does not exists', HttpStatus.BAD_REQUEST);
        }

        const isValid = await this.checkUserPassword(user, credentials.password);
        if (!isValid) {
            throw new HttpException('The email/password combinaison is invalid', HttpStatus.BAD_REQUEST);
        }
    ...
    async validateUser(signedUser: User): Promise<any> {
